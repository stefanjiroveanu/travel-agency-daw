...
import { useImages } from "../../context/unslpashcontext"; // Assuming this is the correct path
import { Link } from "react-router-dom";

export const Locations = () => {
  const [slideIndex, setSlideIndex] = useState(0);
  const { images } = useImages(); // Unsplash images
            ...
            <button key={place.uuid} className="screen4-locationwidgetbutton">
              <Link to="/destinations">
                <img
                  src={
                    images.length > 0
                      ? images[index % images.length].urls.small
                      : null
                  }
                  alt={place.title}
